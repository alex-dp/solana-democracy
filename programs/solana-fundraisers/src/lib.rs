...
pub mod solana_fundraisers {
    ...
    pub fn make_fund(ctx:Context<MakeFund>, fund_id: u16, equal: bool, private: bool, fp_rec_owner: Pubkey, fp_rec_token: Pubkey) -> Result<()> {
        ...
        resize(&mut live_funds.to_account_info(), size_of::<u16>() as i16, signer, &mut sp.to_account_info());
    ...
    pub fn make_partition(ctx:Context<MakePartition>, fund_id: u16, partition_id: u16) -> Result<()> {
        let signer = &mut ctx.accounts.signer;
        let fund = &mut ctx.accounts.fund;
        ...
        resize(&mut fund.to_account_info(), 2 * size_of::<u16>() as i16, signer, &mut sp.to_account_info());

        fund.partitions.push(partition_id);
        fund.points.push(0);
    ...
    pub fn destroy_partition(ctx:Context<DestroyPartition>, fund_id: u16, partition_id: u16) -> Result<()> {
        let fund = &mut ctx.accounts.fund;
    ...
    pub fn set_preference(ctx:Context<SetPreference>, fund_id: u16, recipients: Vec<u16>, points: Vec<u16>) -> Result<()> {
        let fund = &mut ctx.accounts.fund;
        let signer = &mut ctx.accounts.signer;
        ...
        resize(&mut user_preferences.to_account_info(), size_of::<u16>() as i16, signer, &mut sp.to_account_info());
...
pub struct MakeFund<'info> {
    ...
    #[account(
    init,
    seeds = [PARTITION.as_bytes(), &fund_id.to_be_bytes(), &[1]],
    bump,
    space = 8 + Partition::MIN_SIZE,
    payer = signer
    )]
...
pub struct DestroyPartition<'info> {
    ...
    seeds = [FUND.as_bytes(), &fund_id.to_be_bytes()],
    bump,
    constraint = fund.partitions.contains(&partition_id)
    )]
    pub fund: Account<'info, Fund>,
    #[account(
    mut,
...
pub struct DestroyFund<'info> {
    ...
    bump,
    close = signer,
    constraint = fund.partitions.is_empty() && signer.key.eq(&fund.creator)
    )]
    pub fund: Account<'info, Fund>,
...
impl Fund {
    pub const MIN_SIZE: usize = 32 + 32 + 1 + 1 + 1 + 2 + (4 + 2) + (4 + 2);
}
...
impl Partition {
    pub const MIN_SIZE: usize = 32 + 32 + 32 + 16;
}
...
impl Distribution {
    pub const MIN_SIZE: usize = 32 + 16 + 4 + 4 + 4;
    pub const ELEMENT_SIZE: usize = 3;
}
